    ...
    aws_s3: {
      ...
      dev: {
        files: [
          ...
          {expand: true, cwd: 'dist/', src: ['**'], dest: '<%= pkg.name %>/<%= pkg.version.split(".")[0] %>.latest/', params: { CacheControl: 'public, max-age=86400' }},
          // A non-version latest release.
          {expand: true, cwd: 'dist/', src: ['**'], dest: '<%= pkg.name %>/latest/', params: { CacheControl: 'public, max-age=86400' }}
        ]
      }
    },
